...
init url key =
    let
        ( gameWidgetState, gameWidgetCommand ) =
            Widget.init 3000 1000 "gameWidget"
    in
    ( { key = key
...
update msg model =
    case msg of
        ...
        GameMsg gameMsg ->
            ( { model | game = gameUpdate gameMsg model.game }, Cmd.none )

        GameWidgetMsg gameWidgetMessage ->
            let
                ( newWidgetState, widgetCommand ) =
                    Widget.update gameWidgetMessage model.gameWidgetState
            in
    ...
    , body =
        [ Html.div [ Attr.style "text-align" "center", Attr.style "padding-top" "40px" ]
            ...
            , Html.div
                ...
                [ Widget.view model.gameWidgetState (gameView model.game) ]
            ]
        ]
    }


gameView : GameModel -> List (G.Shape FrontendMsg)
gameView model =
